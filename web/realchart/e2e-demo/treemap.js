...
data.forEach((item) => {
    ...
    if (!(group in groupMaxValues) || value > groupMaxValues[group].value) {
        let title = '';
        data.forEach((d) => {
            if (d.value === 0 && item.group === d.id) {
                title = d.name;
            }
        });

        groupMaxValues[group] = { group, value, title };
    }
});

const config = {
    // config와 무관. callback 활용 변수 용도
    vars: {
        groupMaxValues,
        groupSum,
    },
        ...
        pointLabel: {
            ...
            textCallback: (args) => {
                ...
                if (groupMaxValues[args.group].value === args.yValue) {
                    ...
                    return (
                        groupMaxValues[args.group].title +
                        '<br>' +
                        percent.toFixed(2) +
                        '%'
                    );
