    ...
    actions: [
        {
            ...
            action: () => {
                function updateDisplay() {
                    ...
                    const temperatureValue = chart
                        .getGauge('temperature')
                        .get('value');
                    const usageValue = chart.getGauge('usage').get('value');
                    const ramUsageValue = chart
                        .getGauge('ramUsage')
                        .get('value');
                    const randomValue = Math.random() < 0.5 ? -1 : 1;

                    temperature =
                        temperatureValue + randomValue * (Math.random() * 5);
                    usage = usageValue + randomValue * (Math.random() * 5);
                    ramUsage =
                        ramUsageValue + randomValue * (Math.random() * 2);

                    chart
                        .getGauge('temperature')
                        .set('value', Math.round(temperature));
...
function setActions(container) {
    ...
    createListBox(
        ...
        function (e) {
            ...
            config.gauge[0].height =
                pos === 'top' || pos === 'bottom' ? 86 : 65;
    ...
    createCheckBox(
        ...
        'scale.opposite',
        function (e) {
            config.gauge[0].scale.position = _getChecked(e)
                ? 'opposite'
                : 'default';
            chart.load(config);
        },
    ...
    createListBox(
        ...
        function (e) {
            ...
            config.gauge[1].width =
                pos === 'left' || pos === 'right' ? 140 : 65;
    ...
    createCheckBox(
        ...
        'scale2.opposite',
        function (e) {
            config.gauge[1].scale.position = _getChecked(e)
                ? 'opposite'
                : 'default';
            chart.load(config);
        },
