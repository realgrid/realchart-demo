...
const config = {
    ...
    yAxis: [{ template: 'yAxis' }, { template: 'yAxis', col: 1, position: 'opposite' }],
    series: ['pre', 'post'].map((k, i) => {
        return {
            ...
            yAxis: i,
            children: [
                { template: 'area', yField: 'wind', data: data[k]  },
                { template: 'area', yField: 'storage', data: data[k] },
                { template: 'area', yField: 'solar', data: data[k] },
            ]
        }
    }),
    options: {
