    ...
    test.describe('area-multi.html area', () => {
        ...
        test("color", async ({page}) => {
            ...
            let area = await page.evaluate((area) => {
                return window.getComputedStyle(area);
            }, areas[0])

            expect(area.fill).is.equal(firstColor);
            ...
            expect(area.fill).is.equal(secondColor);
        });
        ...
        test("label", async ({page})=> {
            ...
            let viewLabel = await page.evaluate((label)=>{
                ...
                return  new DOMParser().parseFromString(label.innerHTML, 'text/html').body.textContent;
            }, viewLabels[0])

            expect(configLabel).is.equal(viewLabel);

            config = await page.evaluate((obj)=>{
        ...
        test("lineType - default", async ({page}) => {
            ...
            let pathArr = areaPath.split("L").slice(1,-1).map((item) => {
        ...
        test("stepDir - backward", async ({page}) => {
            ...
            let pathYArr = areaPath.split("L").slice(1,-1).map((item) => {
        ...
        test("style", async ({page}) => {
            ...
            expect(area.fill).is.equal(firstColor.fill);
            ...
            expect(area.fill).is.equal(secondColor.fill);
        });
        ...
        test("visible", async({page}) => {
            ...
            const legendStyle = await page.evaluate((legend) => {
                return window.getComputedStyle(legend);
            }, legends[0])

            // visible이 꺼저있는 경우 textDecorationLine이 line-through여야 한다.
            expect(legendStyle.textDecorationLine).is.equal("line-through");
