 ...
 */
describe("equalizer-multi.html test", async function () {

    const url = "http://localhost:6010/realchart/demo/equalizer-multi.html";
    let browser: Browser;
    ...
    it('legend 의 존재 유무와 알맞은 값인지 확인', async () => {
        const page = await PPTester.newPage(browser, url);
        const config: any = await page.evaluate('config');
    ...
    it('xTickLabel 의 값이 알맞은 값인지 확인', async () => {
        ...
        for (let i = 0; i < texts.length; i++) {
            const tickText = await page.evaluate((el) => el.textContent, texts[i]);
