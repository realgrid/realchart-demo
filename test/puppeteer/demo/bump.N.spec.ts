 ...
 */
 describe("bump.html test", async function() {

    const url = "http://localhost:6010/realchart/demo/bump.html";
    let browser: Browser;
    ...
    it('pointLabel', async () => {
        ...
        for(let i = 0; i < pointLabels.length; i++) {
            ...
            for(let j = 0; j < text.length; j ++) {
                const pointText = await page.evaluate((el) => el.textContent, text[j]);
