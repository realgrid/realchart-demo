 ...
 */
 describe("linegroup.html test", async function() {

    const url = "http://localhost:6010/realchart/demo/linegroup.html";
    let browser: Browser;
    ...
    it('tickPoint 의 갯수와 실제 최대 데이터의 갯수가 알맞는지 확인', async () => {
        const page = await PPTester.newPage(browser, url);
        const config: any = await page.evaluate('config');
        ...
        for(let i = 0; i < lineTicks.length; i++) {
            ...
            expect(seriesPoints.length).eq(config.series.children[i].data.length)
        }
