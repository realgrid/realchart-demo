...
/**
 * Puppeteer Tests for lollipop.html
 */
describe("lollipop.html test", async function () {

    const url = "http://localhost:6010/realchart/demo/lollipop.html";
    let browser: Browser;
    ...
    it('init', async () => {
        ...
        // await page.screenshot({path: 'out/ss/lollipop.png'});
        page.close();
    });
    ...
    it('tick 틱의 갯수와 실제 최대 데이터의 갯수가 알맞는지 확인', async () => {
        ...
        expect(xAxisTick.length).eq(count);
    ...
    it('xTickLabel 의 값이 알맞은 값인지 확인', async () => {
        ...
        for (let i = 0; i < texts.length; i++) {
            ...
            expect(textsLabel).eq(config.series.data[i][0]) 
