...
/**
 * Puppeteer Tests for titles.html
 */
 describe("titles.html test", async function() {

    const url = "http://localhost:6010/realchart/demo/titles.html";
    let browser: Browser;
    ...
    it('init', async () => {
        ...
        // await page.screenshot({path: 'out/ss/titles.png'});
        page.close();
    });
    ...
    it('subTitle', async () => {
        ...
        const title = await page.$('.' + TitleView.SUBTITLE_CLASS);
        expect(title).exist;

        const Text = await title.$('text');
        const subtitleText = await page.evaluate((el) => el.textContent, Text);
        expect(subtitleText).eq(config.subtitle.text);
    });
