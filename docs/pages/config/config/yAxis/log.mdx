...
### marginFar: `number{:typescript}`
차트 경계나 뒤쪽 축 사이의 여백 크기.  
`default: 0`  

### marginNear: `number{:typescript}`
Plot 영역이나 앞쪽 축 사이의 여백 크기.  
`default: 0`  

### maxPadding: `number{:typescript}`
...
### nullable: `boolean{:typescript}`
data point의 이 축 값이 NaN일 때도 point를 표시할 지 여부.  
`default: true`  

### padding: `number{:typescript}`
...
### strictMax: `number{:typescript}`
무조건 적용되는 최대값.
즉, 이 값보다 큰 값을 갖는 시리즈 포인트들은 표시되지 않는다.
maxPadding도 적용되지 않는다.  

### strictMin: `number{:typescript}`
무조건 적용되는 최소값.
