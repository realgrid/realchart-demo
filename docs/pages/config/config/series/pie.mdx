...
### autoSlice: `boolean{:typescript}`
클릭한 데이터 포인트를 slice 시킨다.
기존 slice 됐던 포인트는 원복된다.  
`default: true`  

### borderRadius: `number{:typescript}`
`default: 0`  

### center: `string | number{:typescript}`

### centerX: `string | number{:typescript}`

### centerY: `string | number{:typescript}`

### clipped: `boolean{:typescript}`
body 영역을 벗어난 data point view는 잘라낸다.  
`default: false`  

### clockwise: `boolean{:typescript}`
true면 시계 방향으로 회전한다.  
`default: true`  
...
### groupSize: `number{:typescript}`
`default: 1`  

### innerRadius: `string | number{:typescript}`
...
### labelDistance: `number{:typescript}`
`default: 25`  

### legendByPoint: `boolean{:typescript}`
...
### radius: `string | number{:typescript}`
시리즈 원호의 반지름.
...
### sliceDuration: `number{:typescript}`
Slice animation duration.
밀리세컨드(ms) 단위로 지정.  
`default: 300`  

### sliceOffset: `string | number{:typescript}`
`default: '7%'`  

### startAngle: `number{:typescript}`
시리즈 원호 시작 각도.
