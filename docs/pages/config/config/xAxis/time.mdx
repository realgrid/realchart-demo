...
이 값을 지정하지 않으면  padding 에 지정된 값을 따른다.
 endFit 이  **[TICK](/config/config/AxitFit#TICK)** 일 때,
...
계산값과 무관하게 최대값을 지정하려면  strictMax 을 사용한다.  

### minPadding: `number{:js}`[#minPadding]
첫번째 tick 앞쪽에 추가되는 최소 여백을 축 길이에 대한 상대값으로 지정한다.
이 값을 지정하지 않으면  padding 에 지정된 값을 따른다.
 startFit 이  **[TICK](/config/config/AxitFit#TICK)** 일 때,
data point의 최소값과 첫번째 tick 사이에 이미 그 이상의 간격이 존재한다면 무시된다.
 strictMin 가 지정되거나,  minValue 가 계산된 최소값보다 작은 경우에도 이 속성은 무시된다.  
...
계산값과 무관하게 최소값을 지정하려면  strictMin 을 사용한다.  
...
- `'base'` 상대 축의 baseValue 지점에 표시된다.      [주의]   1. 축에 연결된 시리즈들이 BarSeries 계열일 때만 가능하다.  2. 차트의 X축 하나에만 적용할 수 있다. 두번째로 지정된 축의 속성은  NORMAL 로 적용된다.  3. 상대 축이 **linear** 가 아니거나  **[baseValue](/config/config/LinearAxis#baseValue)** 가 min 보다 작거나 max보다 크면 이 값은 무시되고,      NORMAL 로 적용된다.
- `'normal'` X축은 아래쪽에 수평으로, Y축은 왼쪽에 수직으로 표시된다.       **[inverted](/config/config/Chart#inverted)** 이면 Y축이 아래쪽에 수평으로, X축은 왼쪽에 수직으로 표시된다.
- `'opposite'` X축은 위쪽에 수평으로, Y축은 오른쪽에 수직으로 표시된다.       **[inverted](/config/config/Chart#inverted)** 이면 Y축이 위쪽에 수평으로, X축은 오른쪽에 수직으로 표시된다.
...
### startFit[#startFit]
축 시작 위치에 tick 표시 여부.
 strictMin 이 설정되면  AxisFit.VALUE 로 적용된다.  
