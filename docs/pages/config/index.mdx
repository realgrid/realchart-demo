...
**'xAxes'** 또는 **'xAxis'** 이름으로 하나 이상의 X축(들)을 설정한다.\
...
**'yAxes'** 또는 **'yAxis'** 이름으로 하나 이상의 Y축(들)을 설정한다.\
축은 'category', 'linear', 'time', 'log' 등 네가지 종류가 있고,
공통 속성 외에 종류별로 다른 속성들이 추가 포함된다.
...
**'gauges'** 또는 **'gauge'** 이름으로 하나 이상의 게이지(들)을 설정한다.\
둘 이상의 게이지는 배열로 설정한다.
...
시리즈나 게이지가 표시되는 차트 본체 영역에 대한 속성들을 설정한다.\
