  ...
  _parseStructure(obj) {
    switch (obj.kind) {
      ...
      case ReflectionKind.CallSignature:
        console.warn(
          `[WARN] TODO - ${obj.name}:${ReflectionKindString[obj.kind]}`
        );
        break;
      default:
        console.warn(
          `[WARN] Unexpected type ${obj.name}:${ReflectionKindString[obj.kind]}`
        );
        break;
    }
...
class MDGenerater {
  ...
  _styleContent(content) {
    ...
    content = content
      ...
      .replace(
        singleQuoteRegex,
        (match) => `<Content type="expression" value="${match}"></Content>`
      );
  ...
  _setChartContent(dconf) {
    ...
    if (!base && opt) {
      ...
      this.docMap[opt] = {
        ...
        ...this.docMap[opt],
        _content:
          (this.docMap[opt]?._content || '') + `${subtitle}\n${_content}`,
      };
    }
