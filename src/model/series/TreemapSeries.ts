...
import { isArray, isObject, assign, maxv, pickNum } from "../../common/Common";
...
export class TreemapSeriesPoint extends DataPoint {
    ...
    protected override _readArray(series: TreemapSeries, v: any[]): void {
        if (v.length <= 2) {
            ...
            this.id = toStr(v[pickNum(series.idField, 1 + d)]);
            this.group = toStr(v[pickNum(series.groupField, 2 + d)]);
        }
    }
 ...
 * |[x, y, id, group]|형깃 설명 순서대로 값 결정.<br/> 또는 {@link xField} 속성이 숫자이면 x값, {@link yField}는 y값,<br/> {@link idField}는 id값, {@link groupField}는 group값,<br/>.|
 *
 * ###### json 배열
 * |Series 속성|설명|
 * |---|---|
