...
class Base64 {
    ...
    static decode (cipher: string) {
        const tab = Base64.CHARS; 
        const p = Base64.PAD;
        let s = cipher.split(""), out: number[] = [];
        let l = s.length;
...
type CryptorKeyBlock = {[seg: string]: number[]};

export class Cryptor {
    static readonly POW2 = Math.pow(2,2);
    static readonly POW3 = Math.pow(2,3);
    static readonly POW4 = Math.pow(2,4);
    static readonly POW8 = Math.pow(2,8);
    static readonly POW16 = Math.pow(2,16);
    static readonly POW24 = Math.pow(2,24);
    static readonly kb: CryptorKeyBlock = {
    ...
    static union(v: number, box: CryptorKeyBlock){
    ...
    static xor(x: number,y: number): number {
        return (((x>>0x10)^(y>>0x10))<<0x10)|(((x&0xffff)^(y&0xffff))&0xffff);
    }

    static _encryptBlock (obj: Object, box: CryptorKeyBlock) {
        const xor = Cryptor.xor;
        const u = Cryptor.union;
	    let l = obj['left'];
	    let r = obj['right'];
    ...
    static _decryptBlock (obj: Object, box: CryptorKeyBlock) {
        const xor = Cryptor.xor;
        const $ = Cryptor.union;
        let l = obj['left'];
        let r = obj['right'];
    ...
    static _init(key: string): CryptorKeyBlock {
        let bytes = key.split("").map(function(item){
           return item.charCodeAt(0) & 0xff;
        });
        const boxes = Cryptor.kb;
        const eb = Cryptor._encryptBlock;
        //  init the boxes
        let pos=0, data=0, i: number, j: number, l: number;
        let res={ left:0, right:0 };
	    const box = {
            p: boxes.p.slice(0).map(function(item){
                let l = bytes.length, j: number;
                for(j = 0; j < 4; j++) { 
    ...
    static decrypt (ciphertext: string, key: string) {
        const db = Cryptor._decryptBlock;
        let bx = Cryptor._init(key);
        let pt: number[] = [];
        let c = Base64.decode(ciphertext);
        let count = c.length >> 3, pos = 0;
        ...
        if (pt[pt.length - 1] == pt[pt.length - 2] || pt[pt.length - 1] == 0x01){
            let n = pt[pt.length - 1];
            pt.splice(pt.length - n, n);
        }

        //  convert to string
        return String.fromCharCode.apply(null, pt);
...
class Checker {
    ...
    static eqaulsUpperDomain(domain: string, current: string): boolean {
		if (domain == current) {
			return true;
	...
	static _getLDKey(chars: number[]): string {
		let mkey = '';

        for(let i = 0, cnt = chars.length; i < cnt; i++) { 
			mkey += String.fromCharCode(chars[i] >> 1);
		};
		return mkey;
	};

    static includedDomains(licenseDomain: string, currentDomain: string, urlPath: string) {
        let domains = licenseDomain.split(',');
        
        for (let i = 0, cnt = domains.length; i < cnt; i++) {
            const domain = domains[i];

            if (this.eqaulsUpperDomain(domain, currentDomain)) {
                return true;
            }
			if (urlPath && domain && domain[0] === '*' && domain.substring(0, 2) !== '*.') {
				const reg = new RegExp('^\/' + domain.substring(1) + '\/');
				if (reg.exec(urlPath)) {
					return true;
				}
			}
    ...
    static decrypt(encryptedString: string): {[key: string]: string} {
        const decoded = Cryptor.decrypt(encryptedString, _gk+this._getLDKey(Cryptor.kb.k));
		const decObj = {};
		const decArr = decoded.split(";");

		for (let i = 0, cnt = decArr.length; i < cnt; i++) {
			const kv = decArr[i].split("=");
			const key = kv[0];
			const value = kv[1];
...
export const $_lc = function () {
    ...
    try {
        const licServer = licInfo.server.toLowerCase();
        ...
        prodInfo.license = licInfo.lic;
        
        if (licType == LicType.Evaluation || licType == LicType.Limited) {
            const arr = licInfo.expire.split(".").map(function (item) {
                return parseInt(item);
            });
            const d = new Date(arr[0], arr[1], arr[2], 23, 59, 59, 999);
