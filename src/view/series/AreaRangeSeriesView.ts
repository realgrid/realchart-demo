...
export class AreaRangeSeriesView extends LineSeriesBaseView<AreaRangeSeries> {
    ...
    protected override _layoutMarkers(pts: AreaRangeSeriesPoint[], width: number, height: number): void {
        ...
        for (let i = 0, cnt = pts.length; i < cnt; i++) {
            ...
            p.yLow = yOrg - yAxis.getPos(yLen, p.lowValue);
        }
        ...
        for (let i = 0, cnt = pts.length; i < cnt; i++) {
            ...
            if (mv && mv.setVis(!p.isNull && x >= 0 && x <= width && y >= 0 && y <= height)) {
                ...
                mv.setStyle('opacity', (vis || (i == 0 && marker.firstVisible === true) || (i === cnt - 1 && marker.lastVisible === true)) ? '1' : '0');
            }
        }
        // }
    }
